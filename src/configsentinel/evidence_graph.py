...
def build_evidence_graph(report: dict[str, Any]) -> dict[str, Any]:
    ...
    nodes: dict[str, dict[str, Any]] = {f"audit:{audit_id}": {"id": f"audit:{audit_id}", "type": "audit", "label": audit_id, "vendor": audit.get("vendor", "unknown"), "input_sha256": audit.get("input_sha256", "")}}
    ...
    for index, finding in enumerate(report["findings"]):
        ...
        nodes[finding_node] = {"id": finding_node, "type": "finding", "label": finding.get("control_id", finding_id), "status": finding.get("status", "UNKNOWN"), "severity": finding.get("severity", "UNKNOWN"), "confidence": finding.get("confidence", 0.0)}
        edges.append({"from": f"audit:{audit_id}", "to": finding_node, "relation": "produces"})
        control_id = str(finding.get("control_id", "UNKNOWN"))
        control_node = f"control:{control_id}"
        nodes.setdefault(control_node, {"id": control_node, "type": "control", "label": control_id})
        edges.append({"from": finding_node, "to": control_node, "relation": "evaluates"})
        for mapping in finding.get("framework_mappings", []):
            ...
            nodes.setdefault(framework_node, {"id": framework_node, "type": "framework", "label": framework_id})
            edges.append({"from": control_node, "to": framework_node, "relation": "maps_to"})
        for span_index, span in enumerate(finding.get("evidence", [])):
            if not isinstance(span, dict):
                continue
            evidence_node = f"evidence:{finding_id}:{span_index}"
            nodes[evidence_node] = {"id": evidence_node, "type": "evidence", "label": f"L{span.get('start_line', '?')}-L{span.get('end_line', '?')}", "excerpt": str(span.get("excerpt", "")), "redacted": bool(span.get("redacted", True))}
            edges.append({"from": finding_node, "to": evidence_node, "relation": "supported_by"})
