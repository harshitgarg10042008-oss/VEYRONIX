...
def _snapshot_rows(source: Any) -> list[Mapping[str, Any]]:
    ...
    if not isinstance(source, list) or not 1 <= len(source) <= MAX_SNAPSHOTS:
        raise TimeMachineError(f"snapshots must be a list containing 1-{MAX_SNAPSHOTS} entries")
...
def _report_from_row(row: Mapping[str, Any]) -> Mapping[str, Any]:
    ...
    if not isinstance(report, Mapping) or not isinstance(report.get("audit"), Mapping) or not isinstance(report.get("findings"), list):
        raise TimeMachineError("each snapshot requires an audit object and findings list")
...
def _finding_projection(finding: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {"control_id": control_id, "status": status, "severity": str(finding.get("severity", "UNKNOWN"))[:32], "confidence": finding.get("confidence"), "evidence_count": evidence_count}
...
def build_time_machine(source: Any, *, control_id: str | None = None, vendor: str | None = None) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        for raw in report["findings"]:
            ...
            if projected["control_id"] in findings:
                raise TimeMachineError(f"duplicate control in snapshot: {projected['control_id']}")
            findings[projected["control_id"]] = projected
        counts: dict[str, int] = {}
        for finding in findings.values():
            counts[finding["status"]] = counts.get(finding["status"], 0) + 1
        timeline.append({"observed_at": observed_at, "audit_id": _text(audit.get("audit_id", "audit"), "audit.audit_id"), "vendor": report_vendor, "input_sha256": _text(audit.get("input_sha256", "unknown"), "audit.input_sha256", 128), "parser_version": str(audit.get("parser_version", "unknown"))[:64], "findings": dict(sorted(findings.items())), "status_counts": dict(sorted(counts.items()))})
    if not timeline:
        raise TimeMachineError("no snapshots remain after filters")
    timeline.sort(key=lambda item: item["observed_at"])
    control_ids = sorted({control for snapshot in timeline for control in snapshot["findings"]})
    changes: list[dict[str, Any]] = []
    histories: dict[str, list[dict[str, Any]]] = {control: [] for control in control_ids}
    previous: dict[str, str] = {}
    for snapshot in timeline:
        for control in control_ids:
            finding = snapshot["findings"].get(control)
            status = finding["status"] if finding else "MISSING"
            histories[control].append({"observed_at": snapshot["observed_at"], "status": status, "evidence_count": finding.get("evidence_count", 0) if finding else 0})
            if control in previous and previous[control] != status:
                changes.append({"control_id": control, "observed_at": snapshot["observed_at"], "from_status": previous[control], "to_status": status})
    ...
    return {
        ...
        "changes": changes,
        "summary": {"snapshot_count": len(timeline), "control_count": len(control_ids), "change_count": len(changes), "first_observed_at": timeline[0]["observed_at"], "last_observed_at": timeline[-1]["observed_at"]},
        "safety": {"raw_configuration_included": False, "historical_interpolation": False, "live_lookup": False, "verdicts_changed": False, "note": "The time machine replays supplied signed-or-hashed report snapshots only; it does not infer missing periods or establish that a snapshot represents live device state."},
    }
...
__all__ = ["TIME_MACHINE_SCHEMA", "TimeMachineError", "build_time_machine", "load_snapshot_source", "render_time_machine_html", "write_time_machine_html"]
