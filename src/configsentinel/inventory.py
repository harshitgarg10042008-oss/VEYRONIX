...
def _node(raw: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        "id": name,
        "name": name,
        "vendor": str(raw.get("vendor", "unknown"))[:64],
        "role": str(raw.get("role", "unknown"))[:64],
    }
...
def import_inventory(data: bytes | str, *, fmt: str = "json") -> InventoryGraph:
    ...
    try:
        if fmt == "json":
            ...
            node_rows, link_rows = document.get("nodes", []), document.get(
                "links", document.get("edges", [])
            )
        elif fmt == "csv":
    ...
    if (
        not isinstance(node_rows, list)
        or not isinstance(link_rows, list)
        or len(node_rows) > 5000
        or len(link_rows) > 10000
    ):
    ...
    for row in link_rows:
        ...
        source, target = (
            str(row.get("source", row.get("from", ""))).strip(),
            str(row.get("target", row.get("to", ""))).strip(),
        )
        ...
        links.append(
            {
                "source": source,
                "target": target,
                "kind": str(row.get("kind", "connected"))[:64],
            }
        )
