...
class LLMConfig:
    ...
    def from_environment(cls) -> "LLMConfig":
        return cls(
            ...
            max_input_chars=int(os.getenv("CONFIGSENTINEL_LLM_MAX_INPUT_CHARS", "24000")),
            max_output_chars=int(os.getenv("CONFIGSENTINEL_LLM_MAX_OUTPUT_CHARS", "8000")),
...
class OpenAICompatibleProvider:
    ...
    def complete(self, *, system: str, user: str, response_schema: Mapping[str, Any], timeout_s: float) -> str:
        payload = {
            ...
            "temperature": 0,
            "response_format": {"type": "json_schema", "json_schema": {"name": "configsentinel_output", "strict": True, "schema": response_schema}},
        }
...
EXPLANATION_SCHEMA: dict[str, Any] = {
    ...
    "properties": {
        ...
        "evidence_needed": {"type": "array", "items": {"type": "string", "maxLength": 300}, "maxItems": 10},
        "safety_status": {"type": "string", "enum": ["PASS", "REVIEW_REQUIRED", "REJECTED"]},
    },
...
class LLMCopilot:
    ...
    def __init__(self, provider: LLMProvider | None = None, config: LLMConfig | None = None, redactor: SecretRedactor | None = None) -> None:
    ...
    def explain_finding(self, finding: Finding, configuration_context: str) -> LLMExplanation:
        ...
        raw = self.provider.complete(system=system, user=user, response_schema=EXPLANATION_SCHEMA, timeout_s=self.config.timeout_s)
    ...
    def _validate_explanation(result: Any) -> None:
        ...
        if not isinstance(result["evidence_needed"], list) or not all(isinstance(item, str) for item in result["evidence_needed"]):
