...
class AuditCache:
    ...
    def key(
        ...
        *,
        ...
        if (
            not redacted_text.strip()
            or not vendor.strip()
            or not frameworks
            or not rule_pack_version.strip()
        ):
    ...
    def put(self, key: str, report: dict[str, Any]) -> None:
        ...
        if (
            not path.exists()
            and sum(1 for item in self.root.glob("**/*.json") if item.is_file())
            >= self.max_entries
        ):
        ...
        temporary.write_text(
            json.dumps({"cache_key": key, "report": report}, indent=2, sort_keys=True)
            + "\n",
            encoding="utf-8",
        )
