...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="configsentinel",
        description="Audit network configurations and generate review-only remediation previews.",
    )
    ...
    audit.add_argument(
        ...
        dest="frameworks",
        default=None,
        help="framework id; repeat for multiple frameworks (cis-network or nist-800-53)",
    )
    ...
    audit.add_argument(
        "--remediation-out",
        type=Path,
        help="write a non-executable remediation preview",
    )
    audit.add_argument(
        "--diff-out",
        type=Path,
        help="write an evidence-to-command remediation diff preview",
    )
    audit.add_argument(
        "--trail", type=Path, help="append a tamper-evident local audit event"
    )
    audit.add_argument(
        "--signed-out", type=Path, help="write an HMAC-signed JSON evidence envelope"
    )
    audit.add_argument(
        "--signing-key-file", type=Path, help="read the local HMAC signing key"
    )
    audit.add_argument(
        "--approve",
        action="store_true",
        help="acknowledge operator review; still requires --dry-run",
    )
    audit.add_argument(
        "--dry-run",
        action="store_true",
        help="required safety flag; never applies changes",
    )
    audit.add_argument(
        "--policy", type=Path, help="validated local JSON custom policy pack"
    )
    ...
    batch.add_argument(
        ...
        dest="frameworks",
        default=None,
        help="framework id; repeat for multiple frameworks",
    )
    batch.add_argument(
        "--json-out", type=Path, help="write a JSON array of audit reports"
    )
    batch.add_argument(
        "--policy", type=Path, help="validated local JSON custom policy pack"
    )
    gate = sub.add_parser(
        "gitops-check", help="audit configuration files changed between Git revisions"
    )
    ...
    drift = sub.add_parser(
        "drift-check", help="compare a configuration against an approved baseline"
    )
    ...
    gov_request = sub.add_parser(
        "approval-request", help="request independent review for a resource"
    )
    gov_request.add_argument("resource_id")
    gov_request.add_argument("--actor", required=True)
    gov_request.add_argument(
        "--role", choices=("operator", "admin"), default="operator"
    )
    gov_request.add_argument("--reason", default="")
    gov_request.add_argument("--ledger", type=Path, required=True)
    gov_decide = sub.add_parser(
        "approval-decide", help="approve or reject a pending resource review"
    )
    ...
    executive = sub.add_parser(
        "enterprise-report", help="write an executive posture report"
    )
    ...
    history = sub.add_parser(
        "history-analyze", help="analyze a JSON array of serialized audit reports"
    )
    history.add_argument("json_input", type=Path)
    history.add_argument("--out", type=Path, required=True)
    graph = sub.add_parser(
        "evidence-graph", help="project an audit report into an evidence graph"
    )
    graph.add_argument("json_input", type=Path)
    graph.add_argument("--out", type=Path, required=True)
    sensitive = sub.add_parser(
        "sensitive-scan", help="scan a configuration for sensitive markers"
    )
    attest_create = sub.add_parser(
        "attestation-create", help="create a signed configuration attestation"
    )
    ...
    attest_create.add_argument(
        "--reviewer-status",
        choices=("REVIEW_REQUIRED", "APPROVED", "REJECTED"),
        default="REVIEW_REQUIRED",
    )
    attest_create.add_argument("--issued-at")
    attest_verify = sub.add_parser(
        "attestation-verify",
        help="verify and replay a signed configuration attestation",
    )
    ...
    uncertainty = sub.add_parser(
        "uncertainty-budget", help="build an evidence coverage and uncertainty budget"
    )
    uncertainty.add_argument("json_input", type=Path)
    uncertainty.add_argument("--out", type=Path, required=True)
    mutation = sub.add_parser(
        "mutation-lab", help="run bounded semantic mutation tests"
    )
    ...
    mutation.add_argument(
        "--framework", action="append", dest="frameworks", default=None
    )
    mutation.add_argument("--max-mutations", type=int, default=5)
    mutation.add_argument("--out", type=Path, required=True)
    twin = sub.add_parser(
        "assurance-twin", help="build an evidence-first local assurance twin"
    )
    twin.add_argument("topology_input", type=Path)
    twin.add_argument("--report", type=Path)
    twin.add_argument(
        "--finding-asset", action="append", default=[], metavar="FINDING=ASSET"
    )
    twin.add_argument(
        "--add-link", action="append", default=[], metavar="SOURCE=TARGET"
    )
    twin.add_argument(
        "--remove-link", action="append", default=[], metavar="SOURCE=TARGET"
    )
    ...
    intent = sub.add_parser(
        "intent-compile", help="compile resource-level least-privilege intent"
    )
    ...
    apprenticeship = sub.add_parser(
        "apprenticeship-contract",
        help="create a governed unknown-syntax parser contract",
    )
    ...
    apprenticeship_test = sub.add_parser(
        "apprenticeship-test", help="test a parser contract without promoting it"
    )
    ...
    differential = sub.add_parser(
        "differential-test", help="compare explicit vendor variants"
    )
    differential.add_argument(
        "--variant", action="append", required=True, metavar="VENDOR=FILE"
    )
    differential.add_argument(
        "--field", action="append", dest="fields", default=None, choices=SEMANTIC_FIELDS
    )
    differential.add_argument("--case-id", default="case-local")
    differential.add_argument("--out", type=Path, required=True)
    time_machine = sub.add_parser(
        "time-machine", help="replay supplied compliance report snapshots"
    )
    ...
    proof = sub.add_parser(
        "remediation-proof", help="create proof-carrying remediation metadata"
    )
    proof.add_argument("report_input", type=Path)
    proof.add_argument("--out", type=Path, required=True)
    proof_verify = sub.add_parser(
        "remediation-proof-verify", help="verify proof-carrying remediation metadata"
    )
    ...
    exchange = sub.add_parser(
        "audit-exchange", help="create a privacy-preserving audit exchange capsule"
    )
    ...
    exchange_verify = sub.add_parser(
        ...
        help="verify a privacy-preserving audit exchange capsule",
    )
    ...
    reviews = sub.add_parser(
        "reviewer-analytics",
        help="analyze deterministic disagreement across reviewer decisions",
    )
    ...
    freshness = sub.add_parser(
        "assurance-freshness",
        help="assess assurance freshness decay and semantic drift",
    )
    ...
    robustness = sub.add_parser(
        "parser-robustness",
        help="run bounded adversarial robustness cases against a parser",
    )
    ...
    provenance = sub.add_parser(
        "policy-provenance", help="compile hash-linked policy provenance"
    )
    ...
    provenance_verify = sub.add_parser(
        "policy-provenance-verify",
        help="verify policy provenance against source inputs",
    )
    ...
    webhook = sub.add_parser(
        "webhook-enqueue", help="enqueue a redacted audit-completed event locally"
    )
    ...
    exception_add = sub.add_parser(
        "exception-add", help="create a time-bound review exception"
    )
    ...
    exception_approve = sub.add_parser(
        "exception-approve", help="approve a pending time-bound exception"
    )
    ...
    exception_list = sub.add_parser(
        "exception-list", help="list local exceptions and their current status"
    )
    ...
    topology = sub.add_parser(
        "topology-analyze", help="analyze imported topology and render a local explorer"
    )
    ...
    demo = sub.add_parser(
        "demo-mode", help="render a guided local SIH demonstration artifact"
    )
    demo.add_argument("json_input", type=Path)
    demo.add_argument("--after", type=Path)
    demo.add_argument("--out", type=Path, required=True)
    compare = sub.add_parser(
        "audit-compare", help="compare two serialized audit reports"
    )
    compare.add_argument("before", type=Path)
    compare.add_argument("after", type=Path)
    compare.add_argument("--out", type=Path, required=True)
    cached = sub.add_parser(
        "cache-audit", help="audit with a content-addressed local cache"
    )
    ...
    siem = sub.add_parser(
        "siem-export", help="write local SIEM-compatible finding events"
    )
    ...
    backup_create = sub.add_parser(
        "backup-create", help="create an authenticated encrypted JSON backup"
    )
    ...
    backup_create.add_argument(
        "--passphrase-env", default="CONFIGSENTINEL_BACKUP_PASSPHRASE"
    )
    backup_restore = sub.add_parser(
        "backup-restore", help="restore an authenticated encrypted JSON backup"
    )
    ...
    release = sub.add_parser(
        "release-artifacts", help="write deterministic SBOM and release metadata"
    )
...
def run_audit(args: argparse.Namespace) -> int:
    if args.approve and not args.dry_run:
        print(
            "Refusing to proceed: --approve requires --dry-run; no live apply mode exists.",
            file=sys.stderr,
        )
        return 2
    try:
        ...
        client = ConfigSentinelClient(
            engine=DeterministicComplianceEngine(packs), ingestion=service
        )
        result = client.audit_text(
            ingested.redacted_text, vendor=args.vendor, frameworks=frameworks
        )
    ...
    print(
        f"vendor={result.vendor} findings={len(result.findings)} failed={result.failed_count} unknown={len(result.unknown_blocks)} frameworks={','.join(frameworks)}"
    )
    ...
    if args.trail:
        try:
            ...
            return 2
        print(
            f"audit_trail={args.trail} sequence={event.sequence} event_hash={event.event_hash}"
        )
    if args.signed_out:
        ...
        try:
            ...
            envelope = sign_envelope(
                json.loads(client.report_json(result, frameworks=frameworks)), key
            )
            args.signed_out.parent.mkdir(parents=True, exist_ok=True)
            args.signed_out.write_text(
    ...
    if args.report_out:
        write_report(
            result, str(args.report_out), format="markdown", frameworks=frameworks
        )
    ...
    if args.remediation_out or args.diff_out:
        ...
        if args.remediation_out:
            ...
            args.remediation_out.write_text(
                bundle.script, encoding="utf-8", newline="\n"
            )
            print(
                f"remediation_preview={args.remediation_out} steps={bundle.step_count} warnings={len(bundle.warnings)}"
            )
        if args.diff_out:
            args.diff_out.parent.mkdir(parents=True, exist_ok=True)
            args.diff_out.write_text(
                render_diffs(result, bundle), encoding="utf-8", newline="\n"
            )
...
def run_batch(args: argparse.Namespace) -> int:
    try:
        ...
        reports = client.audit_sources(
            str(args.source),
            vendor=args.vendor,
            frameworks=frameworks,
            project_id=str(args.source),
        )
    ...
    for filename, result in reports:
        print(
            f"{filename}\taudit_id={result.audit_id}\tvendor={result.vendor}\tfindings={len(result.findings)}\tfailed={result.failed_count}\tunknown={len(result.unknown_blocks)}"
        )
    if args.json_out:
        import json

        payload = [
            json.loads(client.report_json(result, frameworks=frameworks))
...
def run_release_artifacts(args: argparse.Namespace) -> int:
    try:
        ...
        write_release_artifacts(
            args.root, args.sbom_out, args.metadata_out, manifest=manifest
        )
...
def run_backup_create(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"encrypted_backup={args.out} passphrase_source=environment:{args.passphrase_env}"
    )
    return 0
...
def run_backup_restore(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"restored_backup={args.out} passphrase_source=environment:{args.passphrase_env}"
    )
    return 0
...
def run_cached_audit(args: argparse.Namespace) -> int:
    try:
        ...
        ingested = service.ingest_file(args.file)
        client = ConfigSentinelClient(
            engine=DeterministicComplianceEngine(), ingestion=service
        )
        key = AuditCache.key(
            ingested.redacted_text,
            vendor=args.vendor,
            frameworks=frameworks,
            rule_pack_version=CONTROL_PACK_VERSION,
        )
        report, hit = AuditCache(args.cache_dir).get_or_compute(
            key,
            lambda: report_dict(
                client.audit_text(
                    ingested.redacted_text, vendor=args.vendor, frameworks=frameworks
...
def run_demo_mode(args: argparse.Namespace) -> int:
    try:
        ...
        comparison = (
            compare_reports(report, load_report(args.after)) if args.after else None
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(
            render_guided_demo(report, comparison=comparison),
            encoding="utf-8",
            newline="\n",
        )
    except (OSError, ValueError, DemoError, EvidenceGraphError) as exc:
...
def run_topology_analyze(args: argparse.Namespace) -> int:
    try:
        ...
        payload = analyze_topology(
            graph, finding_assets=finding_assets, depth=args.depth
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(
            json.dumps(payload, indent=2, sort_keys=True) + "\n",
...
def run_exception_add(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"exception={record.exception_id} status={record.status()} verdict_impact=none"
    )
    return 0
...
def run_exception_approve(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"exception={record.exception_id} status={record.status()} approved_by={record.approved_by}"
    )
    return 0
...
def run_exception_list(args: argparse.Namespace) -> int:
    try:
        payload = {
            "schema": "configsentinel.exceptions.v1",
            "exceptions": [item.as_dict() for item in load_exceptions(args.file)],
        }
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(
            json.dumps(payload, indent=2, sort_keys=True) + "\n",
...
def run_risk_prioritize(args: argparse.Namespace) -> int:
    try:
        payload = risk_report(
            load_report(args.json_input), asset_criticality=args.asset_criticality
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(
            json.dumps(payload, indent=2, sort_keys=True) + "\n",
        ...
        return 2
    print(
        f"risk_report={args.out} items={len(payload['items'])} verdict_source={payload['verdict_source']}"
    )
    return 0
...
def run_inventory_import(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"inventory={args.out} nodes={len(graph.nodes)} links={len(graph.links)} discovery=import_only"
    )
    return 0
...
def run_ticket_export(args: argparse.Namespace) -> int:
    try:
        report = load_report(args.json_input)
        rendered = (
            render_ticket_markdown(report)
            if args.format == "markdown"
            else json.dumps(
                build_ticket_payload(report, args.adapter), indent=2, sort_keys=True
            )
            + "\n"
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        ...
        return 2
    print(
        f"ticket_export={args.out} adapter={args.adapter} format={args.format} submission=not_performed"
    )
    return 0
...
def run_webhook_enqueue(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"webhook_queue={args.queue} event={event.event_type} payload_sha256={event.payload_sha256}"
    )
    return 0
...
def run_attestation_create(args: argparse.Namespace) -> int:
    try:
        ...
        attestation = build_attestation(
            report,
            _read_attestation_key(args.key_file),
        ...
        return 2
    print(
        f"attestation={args.out} schema={attestation['schema']} reviewer_status={args.reviewer_status}"
    )
    return 0
...
def run_attestation_verify(args: argparse.Namespace) -> int:
    try:
        report = load_report(args.json_input)
        valid, reason = verify_attestation(
...
def run_uncertainty_budget(args: argparse.Namespace) -> int:
    ...
    print(
        f"uncertainty_budget={args.out} state={assurance['state']} coverage={assurance['evidence_coverage']:.6f} gaps={len(assurance['gaps'])}"
    )
    return 0
...
def run_mutation_lab_command(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"mutation_lab={args.out} passed={report['summary']['passed']} mutations={report['summary']['mutation_count']}"
    )
...
def run_assurance_twin(args: argparse.Namespace) -> int:
    try:
        ...
        for item in args.finding_asset:
            ...
            finding_assets[finding_id] = asset_id
        twin = build_assurance_twin(
            graph,
            report=source_report,
            finding_assets=finding_assets,
            depth=args.depth,
            counterfactual_add=_parse_edge_specs(args.add_link),
            counterfactual_remove=_parse_edge_specs(args.remove_link),
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(
        ...
        return 2
    print(
        f"assurance_twin={args.out} nodes={twin['facts']['node_count']} links={twin['facts']['link_count']} derived_impacts={len(twin['analyses'])}"
    )
    return 0
...
def run_intent_compile(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"intent_compile={args.out} state={compiled['summary']['state']} checks={compiled['summary']['check_count']}"
    )
    return 0
...
def run_apprenticeship_contract(args: argparse.Namespace) -> int:
    try:
        contract = create_contract(
            load_report(args.mapping_input),
            positive_examples=args.positive,
        ...
        return 2
    print(
        f"apprenticeship_contract={args.out} status={contract['promotion']['status']} promoted={contract['promotion']['promoted_into_parser']}"
    )
    return 0
...
def run_apprenticeship_test(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"apprenticeship_test={args.out} status={result['promotion']['status']} promoted={result['promotion']['promoted_into_parser']}"
    )
...
def run_differential(args: argparse.Namespace) -> int:
    try:
        ...
        report = run_differential_test(
            variants, fields=args.fields or SEMANTIC_FIELDS, case_id=args.case_id
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(
            json.dumps(report, indent=2, sort_keys=True) + "\n",
    ...
    print(
        f"differential_test={args.out} equivalent={comparison['equivalent']} semantic_disagreements={comparison['semantic_disagreement_count']} control_disagreements={comparison['control_disagreement_count']}"
    )
    return 0
...
def run_time_machine(args: argparse.Namespace) -> int:
    try:
        machine = build_time_machine(
            load_snapshot_source(args.snapshots_input),
        ...
        return 2
    print(
        f"time_machine={args.out} snapshots={machine['summary']['snapshot_count']} changes={machine['summary']['change_count']}"
    )
    return 0
...
def run_audit_exchange(args: argparse.Namespace) -> int:
    try:
        capsule = build_exchange_capsule(
            ...
            key=_read_exchange_key(args.key_file),
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(
        ...
        return 2
    print(
        f"audit_exchange={args.out} capsule_sha256={capsule['capsule_sha256']} submission=not_performed"
    )
    return 0
...
def run_audit_exchange_verify(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"audit_exchange_verify={args.out} verified={result['verified']} mismatches={len(result['mismatches'])}"
    )
...
def run_reviewer_analytics(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"reviewer_analytics={args.out} reviewers={analytics['summary']['reviewer_count']} disputed={analytics['summary']['disputed_finding_count']} verdicts_changed={analytics['safety']['verdicts_changed']}"
    )
    return 0
...
def run_assurance_freshness(args: argparse.Namespace) -> int:
    try:
        ...
        assessment = build_freshness_assessment(
            report,
        ...
        return 2
    print(
        f"assurance_freshness={args.out} state={assessment['assurance']['state']} freshness={assessment['freshness']['state']} drifted={assessment['drift']['drifted']} verdicts_changed={assessment['safety']['verdicts_changed']}"
    )
    return 0
...
def run_parser_robustness(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"parser_robustness={args.out} cases={pack['summary']['case_count']} crashes={pack['summary']['crash_count']} passed={pack['summary']['passed']} verdicts_changed={pack['safety']['verdicts_changed']}"
    )
...
def run_policy_provenance(args: argparse.Namespace) -> int:
    try:
        ...
        provenance = build_policy_provenance(
            policy, report=report, source_label=args.source_label
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(
        ...
        return 2
    print(
        f"policy_provenance={args.out} rules={provenance['summary']['rule_count']} findings={provenance['summary']['finding_lineage_count']} policy_activated={provenance['safety']['policy_activation']}"
    )
    return 0
...
def run_policy_provenance_verify(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"policy_provenance_verify={args.out} verified={result['verified']} mismatches={len(result['mismatches'])}"
    )
    return 0 if result["verified"] else 1
...
def run_remediation_proof(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"remediation_proof={args.out} state={proof['summary']['state']} proofs={proof['summary']['proof_count']}"
    )
    return 0
...
def run_remediation_proof_verify(args: argparse.Namespace) -> int:
    try:
        result = verify_proof_bundle(
            load_report(args.proof_input), load_report(args.report_input)
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(
            json.dumps(result, indent=2, sort_keys=True) + "\n",
        ...
        return 2
    print(
        f"remediation_proof_verify={args.out} verified={result['verified']} mismatches={len(result['mismatches'])}"
    )
    return 0 if result["verified"] else 1
...
def run_sensitive_scan(args: argparse.Namespace) -> int:
    try:
        ...
        rendered = (
            json.dumps(scan.as_dict(), indent=2, sort_keys=True) + "\n"
            if args.format == "json"
            else render_sensitive_scan(scan)
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        ...
        return 2
    print(
        f"sensitive_scan={args.out} hits={scan.count} input_sha256={scan.input_sha256}"
    )
    return 0
...
def run_evidence_graph(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"evidence_graph={args.out} nodes={len(graph['nodes'])} edges={len(graph['edges'])}"
    )
    return 0
...
def run_history_analyze(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"history_analytics={args.out} reports={analytics['report_count']} dates={len(analytics['timeline'])}"
    )
    return 0
...
def run_executive_report(args: argparse.Namespace) -> int:
    try:
        ...
        ).audit_file(str(args.file), vendor=args.vendor)
        report = build_executive_report(result)
        rendered = (
            render_executive_json(report)
            if args.format == "json"
            else render_executive_markdown(report)
        )
        args.out.parent.mkdir(parents=True, exist_ok=True)
        args.out.write_text(rendered, encoding="utf-8")
    except (OSError, ValueError, RuntimeError) as exc:
        ...
        return 2
    print(
        f"enterprise_report={args.out} posture={report.posture} failed={report.failed} unknown={report.unknown}"
    )
    return 0
...
def run_approval_request(args: argparse.Namespace) -> int:
    try:
        event = ApprovalLedger(args.ledger).request(
            args.resource_id, args.actor, role=Role(args.role), reason=args.reason
        )
    except GovernanceError as exc:
        print(f"Approval request rejected: {exc}", file=sys.stderr)
        return 2
    print(
        f"approval=PENDING_REVIEW resource={event.resource_id} event={event.event_id}"
    )
    return 0
...
def run_approval_decide(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"approval={ledger.status(args.resource_id)} resource={event.resource_id} event={event.event_id}"
    )
    return 0
...
def run_baseline_save(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(
        f"baseline={args.out} input_sha256={result.input_sha256} vendor={result.vendor} findings={len(result.findings)}"
    )
    return 0
...
def run_drift(args: argparse.Namespace) -> int:
    ...
    print(
        f"drift={decision} hash_changed={comparison['hash_changed']} changed_controls={len(comparison['changed_controls'])}"
    )
    if args.json_out:
        args.json_out.parent.mkdir(parents=True, exist_ok=True)
        args.json_out.write_text(
...
def run_gitops(args: argparse.Namespace) -> int:
    try:
        frameworks = normalize_frameworks(args.frameworks)
        result = run_gitops_gate(
            args.repo, args.base, args.head, vendor=args.vendor, frameworks=frameworks
        )
    except (OSError, ValueError, RuntimeError, SourceDiscoveryError) as exc:
    ...
    print(
        f"gitops={decision} changed_files={len(result.changed_files)} findings={len(result.findings)} reason={result.reason}"
    )
    for finding in result.findings:
        lines = ",".join(str(line) for line in finding.evidence_lines) or "none"
        print(
            f"{finding.path}\t{finding.control_id}\t{finding.status}\t{finding.severity}\tevidence={lines}"
        )
    if args.json_out:
        args.json_out.parent.mkdir(parents=True, exist_ok=True)
