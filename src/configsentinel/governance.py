...
class ApprovalLedger:
    ...
    def __init__(self, path: str | Path | None = None) -> None:
        ...
        if (
            ...
            and self.path.stat().st_size > 2 * 1024 * 1024
        ):
        ...
        *,
        ...
        if role not in {Role.OPERATOR, Role.ADMIN}:
            raise GovernanceError(
                "only operators or administrators can request approval"
            )
        event = self._event(
            resource_id,
            actor_id,
            role,
            Action.REQUEST,
            reason or "Submitted for independent review",
        )
        self._append(event)
        return event
    ...
    def decide(
        ...
        *,
        ...
        if role not in {Role.REVIEWER, Role.ADMIN}:
            raise GovernanceError(
                "only reviewers or administrators can decide an approval"
            )
        ...
        if any(
            item.action == Action.REQUEST and item.actor_id == actor_id
            for item in events
        ):
        ...
        return self._append(
            self._event(
                ...
                or (
                    ...
                    if approve
                    else "Rejected during independent review"
                ),
            )
        )
    ...
    def events(self, resource_id: str | None = None) -> tuple[GovernanceEvent, ...]:
        ...
        for line in self.path.read_text(encoding="utf-8").splitlines():
            ...
            try:
                ...
                raise GovernanceError(
                    "governance ledger contains an invalid event"
                ) from exc
    ...
    @staticmethod
    def _event(
        resource_id: str, actor_id: str, role: Role, action: Action, reason: str
    ) -> GovernanceEvent:
