...
def _check_ssh(
...
) -> tuple[FindingStatus, str, tuple[EvidenceSpan, ...]]:
    spans = config.spans_for("management_ssh_enabled") + config.spans_for(
        "management_ssh_version"
    )
    if config.management_ssh_enabled is True and config.management_ssh_version in {
        None,
        "1",
    }:
        return (
            FindingStatus.FAIL,
            "SSH is enabled without an explicitly secure SSH version.",
            spans,
        )
    ...
    return (
        ...
        "Secure SSH state could not be determined from parsed configuration.",
        spans,
    )
...
def _check_telnet(
    ...
    return (
        FindingStatus.UNKNOWN,
        "Telnet state could not be determined from parsed configuration.",
        spans,
    )
...
def _check_logging(
    ...
    return (
        FindingStatus.UNKNOWN,
        "Centralized logging state could not be determined.",
        spans,
    )
...
def _check_http(
    ...
    if config.http_management_enabled is False:
        return (
            FindingStatus.PASS,
            "Plain HTTP management is explicitly disabled.",
            spans,
        )
    return (
        FindingStatus.UNKNOWN,
        "HTTP management state could not be determined.",
        spans,
    )
...
CONTROL_PACK: tuple[ControlDefinition, ...] = (
    ControlDefinition(
        ...
        _check_ssh,
        "Enable SSHv2 and remove legacy SSH versions after change review.",
    ),
    ControlDefinition(
        Control(
            "NET-MGMT-TELNET-001",
        ...
        _check_telnet,
        "Disable Telnet and retain secure management access.",
    ),
    ControlDefinition(
        Control(
            "NET-AUTH-AAA-001",
        ...
        _check_aaa,
        "Configure approved AAA with a tested break-glass process.",
    ),
    ControlDefinition(
        Control(
            "NET-LOG-001",
        ...
        _check_logging,
        "Enable approved logging and route events to a protected collector.",
    ),
    ControlDefinition(
        Control(
            "NET-TIME-001",
        ...
        _check_ntp,
        "Configure approved NTP sources and authentication where supported.",
    ),
    ControlDefinition(
        Control(
            "NET-SNMP-001",
        ...
        _check_snmp,
        "Use SNMPv3 or an approved secure telemetry alternative.",
    ),
    ControlDefinition(
        Control(
            "NET-MGMT-HTTP-001",
        ...
        _check_http,
        "Disable plain HTTP management and use approved TLS settings.",
    ),
)
