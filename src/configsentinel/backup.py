...
def encrypt_backup(payload: Mapping[str, Any], passphrase: str) -> bytes:
    ...
    envelope = {
        "schema": "configsentinel.encrypted-backup.v1",
...
def decrypt_backup(data: bytes, passphrase: str) -> dict[str, Any]:
    ...
    try:
        ...
        payload = json.loads(
            Fernet(_key(passphrase, salt)).decrypt(ciphertext).decode("utf-8")
        )
    except (
        KeyError,
        TypeError,
...
def backup_file(
    input_path: str | Path, output_path: str | Path, passphrase: str
) -> None:
...
def restore_file(
    ...
    destination.write_text(
        json.dumps(payload["document"], indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
