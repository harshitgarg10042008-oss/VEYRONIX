...
def _report_claim(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    reconciliation = _required_mapping(report.get("reconciliation"), "report.reconciliation")
    ...
    if len(input_sha256) != 64 or any(char not in "0123456789abcdef" for char in input_sha256.lower()):
        raise AttestationError("report.audit.input_sha256 must be a lowercase SHA-256 digest")
    ...
    return {
        ...
        "audit": {
            ...
            "framework_registry_version": str(audit.get("framework_registry_version", "")),
...
def _issued_at(value: str | None) -> str:
    if value is not None:
        if not value.endswith("Z") or len(value) < 11:
            raise AttestationError("issued_at must be an ISO-8601 UTC string ending in Z")
        return value
    ...
    if raw_epoch is not None:
        ...
        return _datetime.datetime.fromtimestamp(epoch, tz=_datetime.timezone.utc).replace(microsecond=0).isoformat().replace("+00:00", "Z")
    return DEFAULT_ISSUED_AT


def build_attestation(report: Mapping[str, Any], key: bytes, *, reviewer_status: str = "REVIEW_REQUIRED", issued_at: str | None = None) -> dict[str, Any]:
...
def verify_attestation(attestation: Mapping[str, Any], report: Mapping[str, Any], key: bytes) -> tuple[bool, str]:
    """Verify signature and replay the claim against the supplied report."""
    if not key:
        return False, "verification key cannot be empty"
    try:
        if attestation.get("schema") != ATTESTATION_SCHEMA or attestation.get("algorithm") != ATTESTATION_ALGORITHM:
...
def load_attestation(path: str | Path) -> dict[str, Any]:
    ...
    if source.is_symlink() or not source.is_file() or source.stat().st_size > 512 * 1024:
        raise AttestationError("attestation path is invalid or exceeds the 512 KiB limit")
    try:
