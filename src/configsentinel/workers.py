...
def run_bounded(
    jobs: Iterable[tuple[str, T]],
    worker: Callable[[T], R],
    *,
    max_workers: int = 4,
    max_jobs: int = 1000,
) -> tuple[WorkerResult[R], ...]:
    ...
    with ThreadPoolExecutor(
        max_workers=max_workers, thread_name_prefix="configsentinel"
    ) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                Exception
            ) as exc:  # worker boundary converts failures into explicit job results
                results[job_id] = WorkerResult(
