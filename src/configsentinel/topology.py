...
def blast_radius(
    graph: Mapping[str, Any], *, asset_id: str, depth: int = 1
) -> dict[str, Any]:
    if not 0 <= depth <= 5:
    ...
    return {
        ...
        "distances": distances,
        "safety_note": "Blast radius is a graph-neighborhood review aid; it does not infer traffic flow or exploitability.",
    }
...
def analyze_topology(
    ...
    *,
    ...
    results = {
        finding_id: blast_radius(graph, asset_id=asset_id, depth=depth)
        for finding_id, asset_id in sorted(finding_assets.items())
    }
    impacted = sorted(
        {node_id for item in results.values() for node_id in item["impacted_node_ids"]}
    )
    return {
        "schema": "configsentinel.topology-analysis.v1",
        "source_sha256": graph.get("source_sha256", ""),
        "roots": roots,
        "analyses": results,
        "impacted_node_ids": impacted,
        "safety_note": "Topology is imported or operator-provided; no live discovery, path inference, or remediation is performed.",
    }


def render_topology_html(
    graph: Mapping[str, Any], analysis: Mapping[str, Any] | None = None
) -> str:
