...
def detect_vendor(
    text: str, *, minimum_confidence: float = 0.5, minimum_margin: float = 0.15
) -> VendorDetection:
    if not isinstance(text, str) or not text.strip():
        return VendorDetection(None, 0.0, False, "configuration is empty", ())
    candidates = tuple(
        sorted(
            (_candidate(parser, text) for parser in PARSER_REGISTRY),
    ...
    if top.confidence < minimum_confidence:
        return VendorDetection(
            ...
            top.confidence,
            False,
            "no parser reached the confidence threshold",
            candidates,
        )
    if second and top.confidence - second.confidence < minimum_margin:
        return VendorDetection(
            ...
            top.confidence,
            True,
            "top parser candidates are too close to select safely",
            candidates,
        )
    return VendorDetection(
        top.vendor,
        top.confidence,
        False,
        "selected by deterministic parser signals",
        candidates,
    )
...
def _candidate(parser: VendorParser, text: str) -> VendorCandidate:
    return VendorCandidate(
        parser.plugin_id,
        round(max(0.0, min(1.0, parser.detect(text))), 3),
