...
def audit_from_report(report: Mapping[str, Any]) -> AuditResult:
    ...
    for index, raw in enumerate(report["findings"]):
        ...
        finding_id = _text(
            raw.get("finding_id", f"finding-{index}"), "finding.finding_id"
        )
        ...
        try:
            ...
            raise ProofError(
                f"finding {finding_id} has invalid status, severity, or confidence"
            ) from exc
        ...
        for raw_span in raw.get("evidence", []):
            ...
            try:
                evidence.append(
                    EvidenceSpan(
                        int(raw_span.get("start_line")),
                        int(raw_span.get("end_line")),
                        _text(raw_span.get("excerpt"), "evidence.excerpt", 4096),
                        bool(raw_span.get("redacted", True)),
                    )
                )
            except (TypeError, ValueError) as exc:
                raise ProofError(
                    f"finding {finding_id} contains invalid evidence"
                ) from exc
        findings.append(
            Finding(
                finding_id,
                ...
                confidence,
                tuple(evidence),
                str(raw.get("observed_state", "")),
                str(raw.get("expected_state", "")),
                str(raw.get("rationale", "")),
                raw.get("remediation_preview"),
                bool(raw.get("llm_assisted", False)),
            )
        )
        ...
        if isinstance(raw_span, Mapping):
            unknown_blocks.append(
                EvidenceSpan(
                    int(raw_span.get("start_line")),
                    int(raw_span.get("end_line")),
                    _text(raw_span.get("excerpt"), "unknown_blocks.excerpt", 4096),
                    bool(raw_span.get("redacted", True)),
                )
            )
    return AuditResult(
...
def build_proof_bundle(report: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for step in bundle.steps:
        ...
        if finding is None:
            raise ProofError(
                f"remediation step references missing finding: {step.finding_id}"
            )
        evidence = [
            {
                "start_line": span.start_line,
                "end_line": span.end_line,
                "excerpt_sha256": _excerpt_hash(span),
        ...
        proof_material = {
            ...
            "rollback_sha256": hashlib.sha256(
                step.rollback.encode("utf-8")
            ).hexdigest(),
        }
        proofs.append(
            {
                "proof_id": "proof_" + _hash(proof_material)[:20],
                "finding_id": step.finding_id,
                "control_id": step.control_id,
                "source": {"input_sha256": audit.input_sha256, "evidence": evidence},
                "command_sha256": proof_material["command_sha256"],
                "rollback_sha256": proof_material["rollback_sha256"],
                "preconditions": [
                    f"source_input_sha256 == {audit.input_sha256}",
                    f"control_status == FAIL",
                    "independent operator approval is present",
                ],
                "post_change_verification": {
                    "required": True,
                    "expected_control_status": "PASS",
                    "method": "rerun the deterministic audit against the resulting configuration and review evidence",
                },
                "review": {"requires_human_approval": True, "executable": False},
            }
        )
    proof_id = (
        "remproof_"
    ...
    return {
        ...
        "summary": {
            ...
            "state": (
                "READY_FOR_REVIEW"
                if proofs and not bundle.warnings
                else (
                    "NO_SAFE_STEP"
                    if not proofs and not bundle.warnings
        ...
        "safety": {
            ...
            "note": "Proofs bind deterministic remediation previews to source hashes and evidence references; they do not authorize or execute changes.",
        },
    }


def verify_proof_bundle(
    proof: Mapping[str, Any], report: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    for item in proofs:
        ...
        if not isinstance(review, Mapping):
            review = {}
        if (
            review.get("executable") is not False
            or review.get("requires_human_approval") is not True
        ):
            mismatches.append(f"unsafe review flags: {item.get('finding_id')}")
    return {
        "schema": VERIFY_SCHEMA,
        "proof_bundle_id": str(proof.get("proof_bundle_id", "")),
        ...
        "safety": {
            ...
            "device_connection": False,
            "verdicts_changed": False,
            "note": "Verification checks hash and evidence bindings only; it does not execute commands or prove post-change state.",
        },
    }
