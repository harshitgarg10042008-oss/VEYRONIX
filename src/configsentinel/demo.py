...
def _status_map(report: Mapping[str, Any]) -> dict[str, str]:
    ...
    return {
        str(item.get("control_id", item.get("finding_id", "unknown"))): str(
...
def compare_reports(
    ...
    changes = [
        {
            "control_id": control,
            "before": before_map.get(control, "ABSENT"),
            "after": after_map.get(control, "ABSENT"),
            "changed": before_map.get(control) != after_map.get(control),
        }
        for control in controls
    ]
    return {
        "schema": "configsentinel.audit-comparison.v1",
        "before_audit_id": str(before.get("audit", {}).get("audit_id", "unknown")),
        "after_audit_id": str(after.get("audit", {}).get("audit_id", "unknown")),
        "changes": changes,
        "changed_count": sum(item["changed"] for item in changes),
        "safety_note": "Comparison reflects serialized deterministic reports; it does not prove causality or authorize remediation.",
    }


def render_guided_demo(
    ...
    findings = report.get("findings", [])
    rows = "".join(
        f"<tr><td>{html.escape(str(item.get('control_id', 'unknown')))}</td><td>{html.escape(str(item.get('status', 'UNKNOWN')))}</td><td>{html.escape(str(item.get('severity', 'INFO')))}</td></tr>"
        for item in findings
        if isinstance(item, Mapping)
    )
    comparison_block = (
        f"<pre>{html.escape(json.dumps(comparison, indent=2, sort_keys=True))}</pre>"
        if comparison
        else "<p>No comparison loaded. Add an after-report to show before/after status changes.</p>"
    )
...
def write_demo_html(
    ...
    *,
    ...
    destination.write_text(
        render_guided_demo(report, comparison=comparison), encoding="utf-8"
    )
