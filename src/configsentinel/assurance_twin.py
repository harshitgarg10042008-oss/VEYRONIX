...
def _validate_graph(graph: Mapping[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], set[str]]:
    nodes = graph.get("nodes")
    links = graph.get("links")
    if not isinstance(nodes, list) or not isinstance(links, list) or len(nodes) > MAX_NODES or len(links) > MAX_LINKS:
    ...
    for raw in nodes:
        ...
        normalized_nodes.append({"id": node_id, "name": str(node.get("name", node_id))[:128], "vendor": str(node.get("vendor", "unknown"))[:64], "role": str(node.get("role", "unknown"))[:64], "provenance": "imported"})
    normalized_links: list[dict[str, Any]] = []
    for raw in links:
        link = _object(raw, "topology link")
        source, target = str(link.get("source", "")).strip(), str(link.get("target", "")).strip()
        if source not in node_ids or target not in node_ids or source == target:
            raise AssuranceTwinError("topology links must reference two distinct imported nodes")
        normalized_links.append({"source": source, "target": target, "kind": str(link.get("kind", "connected"))[:64], "provenance": "imported"})
...
def _mapping_pairs(items: list[str], node_ids: set[str]) -> dict[str, str]:
    ...
    for item in items:
        ...
        if not finding_id or asset_id not in node_ids:
            raise AssuranceTwinError("finding-asset mapping references an unknown asset")
...
def _report_findings(report: Mapping[str, Any] | None) -> list[dict[str, Any]]:
    ...
    for raw in findings:
        ...
        finding_id, control_id = str(finding.get("finding_id", "")).strip(), str(finding.get("control_id", "")).strip()
        if not finding_id or not control_id:
            raise AssuranceTwinError("report findings require finding_id and control_id")
        result.append({"finding_id": finding_id, "control_id": control_id, "status": str(finding.get("status", "UNKNOWN")), "severity": str(finding.get("severity", "INFO")), "provenance": "imported_report"})
    return result


def _neighborhood(node_ids: set[str], links: list[dict[str, Any]], root: str, depth: int) -> dict[str, int]:
    if not 0 <= depth <= 5:
...
def build_assurance_twin(graph: Mapping[str, Any], *, report: Mapping[str, Any] | None = None, finding_assets: Mapping[str, str] | None = None, depth: int = 1, counterfactual_add: list[tuple[str, str]] | None = None, counterfactual_remove: list[tuple[str, str]] | None = None) -> dict[str, Any]:
    ...
    mapping = {item["finding_id"]: supplied_mapping[item["finding_id"]] for item in findings if item["finding_id"] in supplied_mapping}
    mapping.update({key: value for key, value in supplied_mapping.items() if key not in finding_ids})

    analyses: list[dict[str, Any]] = []
    for finding_id, asset_id in sorted(mapping.items()):
        distances = _neighborhood(node_ids, links, asset_id, depth)
        analyses.append({"finding_id": finding_id, "root_asset": asset_id, "impacted_node_ids": sorted(distances), "distances": dict(sorted(distances.items())), "provenance": "derived_neighborhood", "confidence": "bounded_graph_only"})
    ...
    for source, target in [*add_edges, *remove_edges]:
        if source not in node_ids or target not in node_ids or source == target:
            raise AssuranceTwinError("counterfactual links must reference two distinct known nodes")
    changed_links = [link for link in links if (link["source"], link["target"]) not in set(remove_edges) and (link["target"], link["source"]) not in set(remove_edges)]
    for source, target in add_edges:
        changed_links.append({"source": source, "target": target, "kind": "counterfactual", "provenance": "counterfactual"})
    counterfactual: dict[str, Any] = {"enabled": bool(add_edges or remove_edges), "added_links": [{"source": source, "target": target} for source, target in add_edges], "removed_links": [{"source": source, "target": target} for source, target in remove_edges], "analyses": []}
    if counterfactual["enabled"]:
        for finding_id, asset_id in sorted(mapping.items()):
            distances = _neighborhood(node_ids, changed_links, asset_id, depth)
            baseline = next(item for item in analyses if item["finding_id"] == finding_id)
            counterfactual["analyses"].append({"finding_id": finding_id, "root_asset": asset_id, "impacted_node_ids": sorted(distances), "newly_impacted_node_ids": sorted(set(distances) - set(baseline["impacted_node_ids"])), "provenance": "derived_counterfactual_neighborhood"})

    unlinked = sorted(finding_id for finding_id in finding_ids if finding_id not in mapping)
    return {
        "schema": TWIN_SCHEMA,
        "source_sha256": str(graph.get("source_sha256", "")),
        "facts": {"nodes": nodes, "links": links, "node_count": len(nodes), "link_count": len(links)},
        "finding_links": [{"finding_id": item["finding_id"], "asset_id": mapping[item["finding_id"]], "provenance": "operator_provided"} for item in findings if item["finding_id"] in mapping],
        "findings": findings,
        "analyses": analyses,
        "unlinked_finding_ids": unlinked,
        "counterfactual": counterfactual,
        "safety": {"live_discovery": False, "traffic_inference": False, "exploitability_inference": False, "remediation_applied": False, "raw_configuration_included": False, "unknown_relations_preserved": True, "note": "Imported facts and bounded graph-derived consequences are labeled separately; absence of a link is not proof of isolation."},
    }
...
__all__ = ["AssuranceTwinError", "TWIN_SCHEMA", "build_assurance_twin", "render_assurance_twin_html", "write_assurance_twin_html"]
