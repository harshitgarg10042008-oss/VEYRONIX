...
def test_custom_required_rule_produces_evidence_backed_pass():
    result = ConfigSentinelClient(engine=DeterministicComplianceEngine((CustomPolicyPack.from_dict(pack()),))).audit_text("version 17.9\nbanner motd ^authorized^\n", vendor="cisco_ios")
    finding = next(item for item in result.findings if item.control_id == "ORG-MGMT-001")
    assert finding.status.value == "PASS"
    assert finding.evidence[0].start_line == 2


def test_custom_forbidden_rule_fails_with_evidence():
    result = ConfigSentinelClient(engine=DeterministicComplianceEngine((CustomPolicyPack.from_dict(pack("forbid")),))).audit_text("version 17.9\nbanner motd ^legacy^\n", vendor="cisco_ios")
    finding = next(item for item in result.findings if item.control_id == "ORG-MGMT-001")
