...
from configsentinel.apprenticeship import (
    ApprenticeshipError,
    create_contract,
    evaluate_contract,
)
...
def test_contract_is_redacted_and_requires_human_promotion():
    contract = create_contract(
        MAPPING,
        positive_examples=["transport input telnet"],
        counterexamples=["transport input ssh"],
    )
...
def test_contract_rejects_bad_counterexamples_and_missing_mapping_fields():
    ...
    redacted = create_contract(
        MAPPING,
        positive_examples=["password super-secret"],
        counterexamples=["transport input ssh"],
    )
...
def test_contract_cli_creates_and_tests_artifacts(tmp_path: Path, capsys):
    ...
    assert (
        main(["apprenticeship-test", str(contract_path), "--out", str(result_path)])
        == 0
    )
    assert "READY_FOR_HUMAN_REVIEW" in capsys.readouterr().out
    assert (
        json.loads(result_path.read_text(encoding="utf-8"))["promotion"][
            "promoted_into_parser"
        ]
        is False
    )
