...
from configsentinel.time_machine import (
    TimeMachineError,
    build_time_machine,
            ...
            "findings": [
                {
                    ...
                    "evidence": [
                        {
                            "excerpt": "raw should not be copied",
                            "start_line": 4,
                            "end_line": 4,
                        }
                    ],
...
def test_time_machine_filters_vendor_and_control_and_rejects_duplicate_times():
    machine = build_time_machine(
        SNAPSHOTS, vendor="cisco_ios", control_id="NET-MGMT-SSH-001"
    )
    assert machine["summary"]["snapshot_count"] == 2
...
def test_time_machine_cli_writes_json_and_html(tmp_path: Path, capsys):
    ...
    assert (
        json.loads(output.read_text(encoding="utf-8"))["summary"]["change_count"] == 1
    )
    assert html.exists()
