...
from configsentinel.exceptions import (
    ExceptionError,
    approve_exception,
    create_exception,
    load_exceptions,
...
def test_exception_lifecycle_is_time_bound(tmp_path: Path):
    ...
    record = create_exception(
        "ex-1", "finding-1", "alice", "maintenance window", FUTURE
    )
...
def test_expired_exception_cannot_be_created_or_approved(tmp_path: Path):
    ...
    path = tmp_path / "exceptions.json"
    path.write_text(
        '[{"exception_id":"ex-old","finding_id":"f","owner":"alice","justification":"old","expires_at":"2000-01-01T00:00:00+00:00"}]',
        encoding="utf-8",
    )
    with pytest.raises(ExceptionError):
