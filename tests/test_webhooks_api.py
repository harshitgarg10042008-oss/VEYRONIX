...
def test_local_webhook_queue_persists_redacted_audit_event(tmp_path: Path):
    report = {"audit": {"audit_id": "audit-1", "vendor": "cisco_ios", "input_sha256": "a" * 64}, "summary": {"failed_count": 1}, "findings": [{"finding_id": "f1", "secret": "must-not-be-forwarded"}]}
    ...
    assert "must-not-be-forwarded" not in (tmp_path / "events.jsonl").read_text(encoding="utf-8")
