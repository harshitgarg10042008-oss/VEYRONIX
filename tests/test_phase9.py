...
from configsentinel.learning import (
    LearningLoopError,
    ReviewDecision,
    UnknownSyntaxQueue,
)
...
def test_unknown_case_and_proposal_are_created():
    ...
    proposal = queue.propose(
        case.case_id,
        interpretation="Enables a secure management service",
...
def test_approval_requires_second_reviewer():
    ...
    proposal = queue.propose(
        case.case_id,
        interpretation="Maps to logging",
        normalized_concept="logging_enabled",
        confidence=0.9,
    )
...
def test_approved_mapping_creates_fixture_and_audit_event(tmp_path: Path):
    ...
    mapping = queue.review(
        ...
        second_reviewer="bob",
        decision=ReviewDecision.APPROVED,
        reason="Verified against vendor documentation",
    )
    ...
    assert (
        queue.regression_fixture(mapping.mapping_id)["source_case_id"] == case.case_id
    )
...
def test_rejection_does_not_create_mapping():
    ...
    proposal = queue.propose(
        case.case_id,
        interpretation="Unclear service",
...
def test_unknown_case_requires_source_provenance():
    ...
    with pytest.raises(LearningLoopError):
        queue.enqueue(
            vendor="cisco_ios",
            parser_version="1",
            evidence=EvidenceSpan(1, 1, " mystery"),
            context="mystery",
            source_sha256="",
        )
