...
def test_framework_aliases_are_normalized():
    assert normalize_frameworks(("cis", "nist", "cis-network")) == ("cis-network", "nist-800-53")


def test_mapping_contains_provenance_and_status():
    result = make_result()
    finding = next(item for item in result.findings if item.control_id == "NET-MGMT-TELNET-001")
...
def test_client_report_methods_match_report_module():
    result = make_result()
    client = ConfigSentinelClient(engine=DeterministicComplianceEngine())
    assert client.report_json(result, frameworks=("cis-network",)) == render_json(result, ("cis-network",))
    assert client.report_markdown(result, frameworks=("cis-network",)) == render_markdown(result, ("cis-network",))
...
def test_report_can_be_written_to_files(tmp_path: Path):
    ...
    write_report(result, str(markdown_path), format="markdown", frameworks=("cis-network",))
    write_report(result, str(json_path), format="json", frameworks=("cis-network",))
    assert markdown_path.exists()
    assert json.loads(json_path.read_text(encoding="utf-8"))["summary"]["finding_count"] == len(result.findings)
