...
from configsentinel.assurance_twin import (
    AssuranceTwinError,
...
GRAPH = {
    ...
    "links": [
        {"source": "edge", "target": "core", "kind": "connected"},
        {"source": "core", "target": "db", "kind": "connected"},
    ],
}
...
def test_twin_counterfactual_is_bounded_and_deterministic():
    first = build_assurance_twin(
        GRAPH,
        finding_assets={"finding-1": "edge"},
        depth=1,
        counterfactual_add=[("edge", "db")],
    )
    second = build_assurance_twin(
        GRAPH,
        finding_assets={"finding-1": "edge"},
        depth=1,
        counterfactual_add=[("edge", "db")],
...
def test_twin_html_is_self_contained_and_does_not_load_external_resources():
    html = render_assurance_twin_html(
        build_assurance_twin(GRAPH, finding_assets={"finding-1": "edge"})
    )
...
def test_twin_cli_writes_json_and_html(tmp_path: Path, capsys):
    ...
    assert (
        json.loads(out_path.read_text(encoding="utf-8"))["counterfactual"]["enabled"]
        is True
    )
