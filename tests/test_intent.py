    ...
    "findings": [
        {
            ...
            "status": "PASS",
            "evidence": [{"start_line": 1, "end_line": 1, "redacted": True}],
        },
        {
            "finding_id": "telnet",
            "control_id": "NET-MGMT-TELNET-001",
            "status": "PASS",
            "evidence": [{"start_line": 2, "end_line": 2, "redacted": True}],
        },
        {
            "finding_id": "http",
            "control_id": "NET-MGMT-HTTP-001",
            "status": "FAIL",
            "evidence": [{"start_line": 3, "end_line": 3, "redacted": True}],
        },
    ]
}

TOPOLOGY = {
    "nodes": [{"id": "jump-host"}, {"id": "edge-management"}],
    "links": [{"source": "jump-host", "target": "edge-management"}],
}
...
def test_intent_without_report_fails_closed_to_review_required():
    ...
    assert all(
        check["observed_status"] == "NO_EVIDENCE" for check in compiled["checks"]
    )
...
def test_intent_rejects_unknown_assets_and_requirements():
    with pytest.raises(IntentError):
        compile_resource_intent(
            INTENT, topology={"nodes": [{"id": "other"}], "links": []}
        )
    with pytest.raises(IntentError):
...
def test_intent_cli_writes_review_artifact(tmp_path: Path, capsys):
    ...
    assert (
        json.loads(out_path.read_text(encoding="utf-8"))["safety"][
            "commands_executable"
        ]
        is False
    )
