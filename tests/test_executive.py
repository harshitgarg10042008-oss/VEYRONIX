...
from configsentinel.executive import build_executive_report, render_executive_json, render_executive_markdown
...
def test_executive_renderers_are_machine_and_human_readable():
    result = ConfigSentinelClient(engine=DeterministicComplianceEngine()).audit_text("version 17.9\nno ip http server\n", vendor="cisco_ios")
    report = build_executive_report(result)
    assert "ConfigSentinel AI executive posture report" in render_executive_markdown(report)
