...
from configsentinel.provenance import (
    ProvenanceError,
...
REPORT = {
    ...
    "findings": [
        {
            "finding_id": "prov-finding",
            "control_id": "CUSTOM-SSH-001",
            "status": "PASS",
            "severity": "HIGH",
            "evidence": [
                {"excerpt": "transport input ssh", "start_line": 4, "end_line": 4}
            ],
        }
    ],
}
...
def test_provenance_is_hash_linked_and_excludes_raw_rule_material() -> None:
    ...
    assert result["summary"] == {
        "rule_count": 1,
        "framework_mapping_count": 2,
        "finding_lineage_count": 1,
        "node_count": 7,
        "edge_count": 6,
    }
...
def test_provenance_cli_compile_and_verify(tmp_path: Path, capsys) -> None:
    ...
    assert (
        main(["policy-provenance", str(policy_path), "--out", str(provenance_path)])
        == 0
    )
