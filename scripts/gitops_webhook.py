...
try:
    ...
    raise SystemExit("FastAPI and uvicorn are required for the GitOps webhook. Run: pip install fastapi uvicorn")
...
async def handle_gitops_webhook(request: Request, x_github_event: str | None = Header(None)) -> dict[str, Any]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return {"status": "passed", "details": "Configuration changes meet compliance requirements."}
        else:
            logger.warning("GitOps gate failed.")
            return {"status": "failed", "details": "Compliance violations detected in the changed files.", "logs": result.stdout}
