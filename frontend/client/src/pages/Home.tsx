...
export default function Home() {
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      setApiOnline(true);
      const response = await fetch(`${API_BASE}/api/audit`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ config_text: DEMO_CONFIGURATION, vendor: "cisco_ios", frameworks: ["cis-network", "nist-800-53"], project_id: "local-dashboard" }) });
  ...
  const runAudit = async () => { setRunning(true); setToast("Submitting redacted configuration to local engine…"); await loadReport(); setRunning(false); };
