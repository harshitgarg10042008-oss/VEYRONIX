...
export default function Home() {
  ...
  const pageContent = () => {
    ...
    return <>
      <div className="overview-hero"><div><SectionLabel>SECURITY ASSURANCE COMMAND CENTER</SectionLabel><h1>Source → Understand → Assess → Decide → Verify → Prove</h1><p>Evidence-backed logic drives every action in this workbench. AI remains advisory, while the rule engine remains the source of truth.</p><div className="hero-meta"><span><span className="signal signal-teal" /> {apiOnline ? "LOCAL API ONLINE" : "OFFLINE FIXTURE"}</span><span><LockKeyhole size={12} /> NO LIVE DEVICE</span><span><Fingerprint size={12} /> SHA-256 BOUND</span></div></div><div className="hero-score"><span>POSTURE SCORE</span><strong>{score}<small>/100</small></strong><div className="score-track"><i style={{ width: `${score}%` }} /></div><span>{score >= 80 ? "operationally healthy" : score >= 60 ? "monitor with review" : "requires intervention"}</span></div></div>
      <div className="overview-actions"><div><SectionLabel>CURRENT WORKSPACE</SectionLabel><strong>{selectedFileName}</strong><span>{loading ? "Loading evidence report…" : `${report.summary.finding_count} findings · ${report.summary.failed_count} failures · ${report.summary.unknown_count} unknown`}</span></div><div className="action-row">{auditActions}<button type="button" className="button button-secondary" onClick={() => navigate("/review-queue")}><CircleHelp size={15} /> Review queue {reviewFindings.length ? `(${reviewFindings.length})` : ""}</button></div></div>
      <div className="metrics-grid"><Metric label="FAILURES" value={report.summary.failed_count.toString().padStart(2, "0")} note="require attention" tone={report.summary.failed_count ? "danger" : "safe"} /><Metric label="UNKNOWN" value={report.summary.unknown_count.toString().padStart(2, "0")} note="review before trust" tone="warn" /><Metric label="EVALUATED" value={report.summary.evaluated_count.toString().padStart(2, "0")} note="rule-based results" /><Metric label="SAVED AUDITS" value={history.length.toString().padStart(2, "0")} note="browser-local history" /></div>
      <div className="two-column"><section className="panel"><div className="panel-head"><div><SectionLabel>ASSURANCE JOURNEY</SectionLabel><h2>Decision flow</h2></div></div><div className="guide-grid" style={{ gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 12, padding: 16 }}>{[["SOURCE", "Start from a known configuration or captured evidence bundle."], ["UNDERSTAND", "Parse the input and determine the correct vendor and control context."], ["ASSESS", "Apply the active rule pack to current posture and evidence."], ["DECIDE", "Resolve fails, unknowns, and approval status before acting."], ["VERIFY", "Check change history, drift, freshness, and proof integrity."], ["PROVE", "Export signed evidence and keep the decision audit-ready."]].map(([step, description], index) => <div key={step} className={`guide-step ${index === 0 ? "active" : ""}`} style={{ minHeight: 100 }}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{step}</strong><small>{description}</small></div></div>)}</div></section><section className="panel"><div className="panel-head"><div><SectionLabel>Next Best Action</SectionLabel><h2>Current priority</h2></div></div><div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}><div className="queue-banner" style={{ margin: 0 }}><div className="queue-icon"><CircleHelp size={22} /></div><div><strong>{failedFindings.length ? `Address ${failedFindings.length} failed control(s)` : "No rule-based failures remain"}</strong><span>{failedFindings.length ? "Review the failing evidence, confirm intent, then route the proposal through the approval boundary." : "The current posture is stable under the active rule pack and evidence set."}</span></div><button type="button" className="button button-secondary" onClick={() => navigate("/review-queue")}>Open queue <ArrowRight size={15} /></button></div><div className="remediation-guard" style={{ margin: 0 }}><div className="guard-icon"><LockKeyhole size={21} /></div><div><strong>Deterministic engine holds the verdict; AI explains the path.</strong><span>All decisions remain evidence-bound, reviewable, and non-executable until an operator approves a bounded remediation preview.</span></div></div></div></section></div>
      <div className="two-column"><section className="panel"><div className="panel-head"><div><SectionLabel>POSTURE / LATEST REPORT</SectionLabel><h2>{selectedFileName}</h2></div><div className="action-row"><button type="button" className="button button-secondary" onClick={() => setShowFilters((value) => !value)}><SlidersHorizontal size={15} /> Filters {filterCount ? `(${filterCount})` : ""}</button><button type="button" className="button button-secondary" onClick={() => exportReport()}><Download size={15} /> Export PDF</button></div></div>{showFilters && <FilterBar severity={severityFilter} setSeverity={setSeverityFilter} status={statusFilter} setStatus={setStatusFilter} framework={frameworkFilter} frameworkOptions={frameworkOptions} setFramework={setFrameworkFilter} reset={resetFilters} />}<div className="findings-table"><div className="table-head"><span>CONTROL / EVIDENCE</span><span>VENDOR</span><span>SEVERITY</span><span>STATUS</span></div>{visibleFindings.length === 0 ? <EmptyState title="No findings match this view" detail="Change the filters or run a local audit to populate the evidence table." /> : visibleFindings.slice(0, 6).map((finding) => <button type="button" className={`finding-row ${finding.finding_id === selected?.finding_id ? "finding-selected" : ""}`} key={finding.finding_id} onClick={() => setSelectedId(finding.finding_id)}><span className="finding-main"><span className={`finding-symbol symbol-${finding.status.toLowerCase()}`}>{finding.status === "FAIL" ? "!" : finding.status === "PASS" ? "✓" : "?"}</span><span><strong>{finding.control_id}</strong><small>{finding.observed_state || finding.rationale}</small><code>{evidenceLine(finding)} <span>{frameworkText(finding)}</span></code></span></span><span className="vendor-label">{vendorLabel(report.audit.vendor)}</span><Severity value={finding.severity} /><StatusPill status={finding.status} /></button>)}</div></section><aside className="panel"><div className="panel-head"><div><SectionLabel>TRUST BOUNDARY</SectionLabel><h2>Evidence boundary</h2></div></div><div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}><div className="queue-banner" style={{ margin: 0 }}><div className="queue-icon"><LockKeyhole size={20} /></div><div><strong>AI is advisory. The rule engine decides.</strong><span>Source files, rule packs, and proof receipts remain in the local boundary. Uncertain findings are visible and never silently upgraded.</span></div></div></div></aside></div>
      <div className="two-column"><TrendPanel history={history} onSelect={(entry) => selectHistory(entry)} /><HistoryPanel history={history} onSelect={(entry) => selectHistory(entry)} onDelete={(entry) => deleteHistory(entry)} onExport={(entry) => exportReport(entry.report, entry.fileName)} /></div>
    </>;
  };
...
function FilterBar({ severity, setSeverity, status, setStatus, framework, frameworkOptions, setFramework, reset }: { severity: SeverityValue | "ALL"; setSeverity: (value: SeverityValue | "ALL") => void; status: FindingStatus | "ALL"; setStatus: (value: FindingStatus | "ALL") => void; framework: string; frameworkOptions: string[]; setFramework: (value: string) => void; reset: () => void }) {
  return (
    <div className="filter-bar">
      ...
      <label>
        Status
        <select value={status} onChange={(event) => setStatus(event.target.value as FindingStatus | "ALL")}>
          <option value="ALL">All statuses</option>
          {["FAIL", "PASS", "UNKNOWN", "REVIEW_REQUIRED"].map((value) => <option key={value} value={value}>{value}</option>)}
        </select>
      </label>
      <label>
        Framework
        <select value={framework} onChange={(event) => setFramework(event.target.value)}>
          ...
          {frameworkOptions.map((value) => <option key={value} value={value}>{value.replaceAll("-", " ").toUpperCase()}</option>)}
        </select>
      </label>
