...
function detectVendor(configText: string): "cisco_ios" | "junos" | "firewall_generic" {
  const text = configText.toLowerCase();
  if (/^\s*set\s+(system|interfaces|security|firewall)/m.test(text) || /junos|juniper|ge-\d/.test(text)) return "junos";
  if (/pan-os|palo alto|security-policy|firewall policy|checkpoint|fortigate|set service/.test(text)) return "firewall_generic";
  return "cisco_ios";
}
...
function FindingsTrend({ history, onSelect }: { history: AuditHistoryEntry[]; onSelect: (entry: AuditHistoryEntry) => void }) {
  ...
  return <div className="trend-panel ruled-panel"><div className="panel-head"><div><div className="panel-eyebrow">HISTORY / {history.length} SNAPSHOT{history.length === 1 ? "" : "S"}</div><h3>Finding trend</h3></div><div className="trend-legend"><span><i className="trend-fail" /> failures</span><span><i className="trend-unknown" /> unknown</span></div></div>{history.length === 0 ? <div className="empty-state">Run an audit or upload a configuration to build the local trend.</div> : <div className="trend-chart-wrap"><svg className="trend-chart" viewBox="0 0 100 100" role="img" aria-label="Interactive trend of failures and unknown findings over saved audits"><path d="M8 86 H92" className="trend-axis" /><path d="M8 18 V86" className="trend-axis" /><path d={path("failures")} className="trend-line trend-line-fail" /><path d={path("unknown")} className="trend-line trend-line-unknown" />{points.map((point) => <g key={point.entry.id} tabIndex={0} role="button" aria-label={`${point.entry.fileName}: ${point.failures} failures, ${point.unknown} unknown`} onClick={() => onSelect(point.entry)} onMouseEnter={() => setHoveredId(point.entry.id)} onMouseLeave={() => setHoveredId(null)} onFocus={() => setHoveredId(point.entry.id)} onBlur={() => setHoveredId(null)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onSelect(point.entry); }}><circle cx={point.x} cy={86 - (point.failures / max) * 68} r="2.4" className="trend-point trend-point-fail" /><circle cx={point.x} cy={86 - (point.unknown / max) * 68} r="2.4" className="trend-point trend-point-unknown" /><title>{new Date(point.entry.capturedAt).toLocaleString()} · {point.failures} failures · {point.unknown} unknown</title></g>)}</svg>{hoveredId && (() => { const hovered = history.find((entry) => entry.id === hoveredId); return hovered ? <div className="trend-tooltip"><strong>{hovered.fileName}</strong><span>{new Date(hovered.capturedAt).toLocaleString()}</span><span><b>{hovered.report.summary.failed_count}</b> failures · <b>{hovered.report.summary.unknown_count}</b> unknown</span><small>Click point to load this audit</small></div> : null; })()}<div className="trend-labels"><span>{new Date(points[0].entry.capturedAt).toLocaleDateString()}</span><span>Click a point to load snapshot</span><span>{new Date(points.at(-1)!.entry.capturedAt).toLocaleDateString()}</span></div></div>}</div>;
}
...
function HistoryPanel({ history, onSelect, onDelete, onExport }: { history: AuditHistoryEntry[]; onSelect: (entry: AuditHistoryEntry) => void; onDelete: (entry: AuditHistoryEntry) => void; onExport: (entry: AuditHistoryEntry) => void }) {
  return <div className="history-panel ruled-panel"><div className="panel-head"><div><div className="panel-eyebrow">LOCAL STORAGE / {history.length} SAVED</div><h3>Audit history</h3></div><span className="history-retention">latest 20</span></div>{history.length === 0 ? <div className="empty-state">No saved audits yet. Run an audit or upload a configuration.</div> : <div className="history-list">{history.map((entry) => <div className="history-row" key={entry.id}><button className="history-select" onClick={() => onSelect(entry)}><span className="history-date">{new Date(entry.capturedAt).toLocaleString()}</span><strong>{entry.fileName}</strong><small>{entry.report.audit.vendor} · {entry.report.summary.failed_count} failures · {entry.report.summary.unknown_count} unknown</small></button><button className="history-action" aria-label={`Export ${entry.fileName}`} onClick={() => onExport(entry)}><Download size={14} /></button><button className="history-action history-delete" aria-label={`Delete ${entry.fileName}`} onClick={() => onDelete(entry)}><X size={14} /></button></div>)}</div>}</div>;
}
...
export default function Home() {
  ...
  const loadReport = async (configText = DEMO_CONFIGURATION, fileName = "bundled-fixture.conf", vendor = "cisco_ios") => {
    ...
    try {
      ...
      setApiOnline(true);
      const response = await fetch(`${API_BASE}/api/audit`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ config_text: configText, vendor, frameworks: ["cis-network", "nist-800-53"], project_id: fileName }) });
  ...
  const handleFileUpload = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try { const text = await file.text(); if (!text.trim()) throw new Error("empty"); const vendor = detectVendor(text); setToast(`Detected ${vendor} · submitting ${file.name}…`); await loadReport(text, file.name, vendor); }
  ...
  const deleteHistory = (entry: AuditHistoryEntry) => { const next = history.filter((item) => item.id !== entry.id); setHistory(next); persistHistory(next); if (report.audit.audit_id === entry.report.audit.audit_id) { const replacement = next[0]; if (replacement) selectHistory(replacement); else { setReport(fallbackReport); setSelectedId(""); } } setToast(`Deleted local snapshot · ${entry.fileName}`); };
  const exportHistoryReport = (entry: AuditHistoryEntry) => { const pdf = new jsPDF({ unit: "pt", format: "a4" }); pdf.setFillColor(45, 48, 43); pdf.rect(0, 0, 595, 82, "F"); pdf.setTextColor(244, 241, 232); pdf.setFontSize(20); pdf.text("CONFIGSENTINEL AI", 40, 42); pdf.setFontSize(9); pdf.text("LOCAL HISTORY SNAPSHOT · EVIDENCE REPORT", 40, 60); pdf.setTextColor(45, 48, 43); pdf.setFontSize(14); pdf.text(`${entry.fileName} · ${new Date(entry.capturedAt).toLocaleString()}`, 40, 120); pdf.setFontSize(10); pdf.text(`Audit ${entry.report.audit.audit_id} · ${entry.report.audit.vendor}`, 40, 144); pdf.text(`Failures ${entry.report.summary.failed_count} · Unknown ${entry.report.summary.unknown_count} · Findings ${entry.report.summary.finding_count}`, 40, 162); let y = 194; entry.report.findings.forEach((finding, index) => { const line = `${index + 1}. ${finding.control_id} · ${finding.status} · ${finding.severity}`; pdf.text(pdf.splitTextToSize(line, 515), 40, y); y += 18; pdf.text(pdf.splitTextToSize(`Evidence: ${evidenceText(finding)} · ${evidenceLine(finding)}`, 515), 40, y); y += 30; if (y > 760) { pdf.addPage(); y = 44; } }); pdf.setFontSize(9); pdf.text("Safety note: historical evidence only; no device mutation is authorized.", 40, Math.min(y, 780)); pdf.save(`configsentinel-history-${entry.report.audit.audit_id}.pdf`); setToast(`Exported history snapshot · ${entry.fileName}`); };
  ...
  const exportPdf = () => {
    ...
    pdf.setTextColor(244, 241, 232); pdf.setFontSize(20); pdf.text("CONFIGSENTINEL AI", 40, 42); pdf.setFontSize(9); pdf.text("LOCAL SECURITY WORKBENCH · EVIDENCE REPORT", margin, 62);
    ...
    pdf.save(`configsentinel-${report.audit.audit_id || "posture"}.pdf`); setToast(`PDF exported · ${visibleFindings.length} visible finding(s)`);
  };

  return <main className="app-shell">
    <aside className="sidebar"><div className="brand-lockup"><div className="brand-mark-wrap"><img src={logo} alt="ConfigSentinel AI mark" className="brand-mark" /></div><div><div className="brand-name">CONFIGSENTINEL</div><div className="brand-sub">AI · OFFLINE SECURITY WORKBENCH</div><div className="brand-team">BY VEYRONIX</div></div></div>
