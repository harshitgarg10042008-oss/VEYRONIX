...
export default function Home() {
  ...
  const exportHistoryReport = (entry: AuditHistoryEntry) => { const pdf = new jsPDF({ unit: "pt", format: "a4" }); pdf.setFillColor(45, 48, 43); pdf.rect(0, 0, 595, 82, "F"); pdf.setTextColor(244, 241, 232); pdf.setFontSize(20); pdf.text("VEYRONIX", 40, 40); pdf.setFontSize(9); pdf.text("LOCAL HISTORY SNAPSHOT · EVIDENCE REPORT", 40, 60); pdf.setTextColor(45, 48, 43); pdf.setFontSize(14); pdf.text(`${entry.fileName} · ${new Date(entry.capturedAt).toLocaleString()}`, 40, 120); pdf.setFontSize(10); pdf.text(`Audit ${entry.report.audit.audit_id} · ${entry.report.audit.vendor}`, 40, 144); pdf.text(`Failures ${entry.report.summary.failed_count} · Unknown ${entry.report.summary.unknown_count} · Findings ${entry.report.summary.finding_count}`, 40, 162); let y = 194; entry.report.findings.forEach((finding, index) => { const line = `${index + 1}. ${finding.control_id} · ${finding.status} · ${finding.severity}`; pdf.text(pdf.splitTextToSize(line, 515), 40, y); y += 18; pdf.text(pdf.splitTextToSize(`Evidence: ${evidenceText(finding)} · ${evidenceLine(finding)}`, 515), 40, y); y += 30; if (y > 760) { pdf.addPage(); y = 44; } }); pdf.setFontSize(9); pdf.text("Safety note: historical evidence only; no device mutation is authorized.", 40, Math.min(y, 780)); pdf.save(`veyronix-history-${entry.report.audit.audit_id}.pdf`); setToast(`Exported history snapshot · ${entry.fileName}`); };
